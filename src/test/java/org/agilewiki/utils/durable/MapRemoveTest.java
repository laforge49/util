...
public class MapRemoveTest extends TestCase {
    public void test() throws Exception {
        assertEquals(DurableMapNode.MAP_NIL, DurableMapNode.MAP_NIL.remove("", 0, 1));
        DurableMapNode.MAP_NIL.clearMap(1);
        assertEquals(1, DurableMapNode.MAP_NIL.set("1", "a", 1).size(1));

        DurableMapNode m1 = DurableMapNode.MAP_NIL.add("1", "a", 2);
        m1 = m1.remove("1", 0, 3);

        DurableMapNode m2 = m1.copyMap();
        m2.clearMap(3);

...
public class MapRemoveTest extends TestCase {
    public void test() throws Exception {
        assertEquals(LazyDurableMapNode.MAP_NIL, LazyDurableMapNode.MAP_NIL.remove("", 0, 1));
        LazyDurableMapNode.MAP_NIL.clearMap(1);
        assertEquals(1, LazyDurableMapNode.MAP_NIL.set("1", "a", 1).size(1));

        LazyDurableMapNode m1 = LazyDurableMapNode.MAP_NIL.add("1", "a", 2);
        m1 = m1.remove("1", 0, 3);

        LazyDurableMapNode m2 = m1.copyMap();
        m2.clearMap(3);
